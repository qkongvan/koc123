...
import { getStoredKeys, saveStoredKeys, testApiKey } from './services/keyService';
...
const App: React.FC = () => {
  ...
  const [apiCheckResult, setApiCheckResult] = useState<{ text?: { success: boolean; error?: string }; image?: { success: boolean; error?: string } } | null>(null);
  ...
  const handleCheckApi = async () => {
    ...
    const textKeys = tempTextKeys.split('\n').map(k => k.trim()).filter(k => k.length > 0);
    const imageKeys = tempImageKeys.split('\n').map(k => k.trim()).filter(k => k.length > 0);
    
    const results: any = {};
    
    // Test first text key
    if (textKeys.length > 0) {
      results.text = await testApiKey(textKeys[0], 'text');
    } else {
      // Fallback to env if empty? No, user wants to test what's in the box.
      results.text = { success: false, error: "Chưa nhập API Key 01" };
    }
    
    // Test first image key
    if (imageKeys.length > 0) {
      results.image = await testApiKey(imageKeys[0], 'image');
    } else {
      results.image = { success: false, error: "Chưa nhập API Key 02" };
    }
    
    setApiCheckResult(results);
        ...
        <nav className={`${theme.colors.cardBackground} border-b ${theme.colors.borderLight} sticky top-0 z-[100] h-20 flex items-center shadow-sm`}>
          <div className="max-w-7xl mx-auto w-full px-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                ...
                <button 
                  onClick={() => {
                    setTempTextKeys(getStoredKeys('text').join('\n'));
                    setTempImageKeys(getStoredKeys('image').join('\n'));
                    setApiCheckResult(null);
            ...
            <div className="relative bg-white w-full max-w-lg rounded-[2rem] p-8 !mt-[50px] shadow-2xl border border-slate-200 animate-fadeIn">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <textarea value={tempTextKeys} onChange={(e) => { setTempTextKeys(e.target.value); setApiCheckResult(null); }} placeholder="Paste danh sách API Key cho Text tại đây..." className={`w-full h-32 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-mono ${theme.colors.inputFocus} outline-none resize-none`} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">API Key 02</label>
                  <textarea value={tempImageKeys} onChange={(e) => { setTempImageKeys(e.target.value); setApiCheckResult(null); }} placeholder="Paste danh sách API Key cho Image tại đây..." className={`w-full h-32 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-mono ${theme.colors.inputFocus} outline-none resize-none`} />
                </div>

                {apiCheckResult && (
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-2">
                    <div className="flex items-center gap-2 text-[11px] font-bold">
                      <div className={`w-2 h-2 rounded-full ${apiCheckResult.text?.success ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <span className="text-slate-500 uppercase">API 1:</span>
                      <span className={apiCheckResult.text?.success ? 'text-green-600' : 'text-red-600'}>
                        {apiCheckResult.text?.success ? 'Hoạt động tốt' : `Lỗi: ${apiCheckResult.text?.error}`}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-[11px] font-bold">
                      <div className={`w-2 h-2 rounded-full ${apiCheckResult.image?.success ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <span className="text-slate-500 uppercase">API 2:</span>
                      <span className={apiCheckResult.image?.success ? 'text-green-600' : 'text-red-600'}>
                        {apiCheckResult.image?.success ? 'Hoạt động tốt' : `Lỗi: ${apiCheckResult.image?.error}`}
                      </span>
                    </div>
                    {apiCheckResult.text?.success && apiCheckResult.image?.success && (
                      <div className="pt-2 text-center text-[10px] font-black text-green-600 uppercase tracking-tight">
                        Hiện API 1 và API 2 đang hoạt động
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex flex-col gap-3">
                  <button 
                    ...
                    className={`w-full py-4 bg-slate-900 text-white font-black rounded-2xl text-xs uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-2 disabled:opacity-50`}
                  >
                    {isCheckingApi ? (
                      <>
